...
Vprog = "polizas.p".
{&out} "nom:" vnombre.
IF Vnombre <> ? AND Vnombre <> "!" THEN DO:
    {&OUT} "inicio:" STRING(TIME,"hh:mm:ss").
   IF vnombre <> "" AND SEARCH("C:\TEMP\VER11\" + Vnombre + ".DAT") <> ? THEN DO:
      
      INPUT FROM VALUE("C:\TEMP\VER11\" + Vnombre + ".DAT").
      REPEAT:
   ...
   END.
   ELSE DO:
   RUN C:\proyectos\Galeno\Programas\p-ConsultaPorNombreBeneficiario2.p (VNombre, OUTPUT TABLE tt-ConsPorPoliza, OUTPUT opi-Error, OUTPUT opc-MsgError).
   {&OUT}  "opc-MsgError:" opc-MsgError.
   OUTPUT TO VALUE("C:\TEMP\VER11\" + Vnombre + ".DAT").
...
{&OUT} '
    ...
    <form action="" method="POST" name="SearchForm1">
       ...
       <p><strong>Nombre Completo Derecho Habiente
       <input type="text" size="15" name="vNombre" value="' Vfcve-poliza '">
...
OUTPUT STREAM pagina TO pagina.txt.
FOR FIRST tt-ConsPorPoliza NO-LOCK :
