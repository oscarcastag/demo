...
function getAlumnos() {
    ...
    $.ajax({
        type: "GET",
        url: host + "/Test/gettest.php", //host variable global
        data: "action=gettest&jwt=" + jwt.toLocaleString(),        
        success: function (text) {
            //console.log(text);
			text = JSON.parse(text);
            var date = text;
			...
			for (x in date) {
				txt += '<tr>';
				txt += '<td><a href="updalumno.php?Proveedor=' + date[x].Proveedor;
				txt += '&Estatus=' + date[x].Marcado;
				txt += '&Nombre=' + date[x].Nombre;
				txt += '">Modificar</a>';
				txt += ' / <a href="delalumno.php?Proveedor=' + date[x].Proveedor;
				txt += '&Estatus=' + date[x].Marcado;
				txt += '&Nombre=' + date[x].Nombre;
				txt += '">Eliminar</a></td>';
